...
load_raw_death_data <- function(folder, include_prelim, file_years = NULL) {
  # Step 0: Identify all Death Statistical Files in folder
  files <- dir_ls(path = folder, recurse = FALSE) %>%
    path_file() %>%
    list(files = .) %>%
    as_tibble(.) %>%
    ...
    mutate(
      ...
      final = str_detect(file, pattern = "Q", negate = TRUE)
    ) %>%
    # Arrange by Year
    arrange(year) %>%
  ...
  # Step 2: Extract Death File Names
  ...
  for (file in death_files) {
    ## Extract File Year
    file_year <- str_extract(file, "\\d{4}")
    ...
    ## Store Loaded Data
    death_list[[file_year]] <- import(
      file = here(folder, file)
    ) %>%
      clean_names()
  }
